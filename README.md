...
Uber의 운행 수가 저렇게 많은데도 불구하고 평균 도착소요시간이 5분도 안 걸린다는 것은 추가 변수도 고려해봐야겠지만 택시 매칭 시스템이 잘되어 있기 때문일 것으로 판단된다.
...
![image](https://user-images.githubusercontent.com/22818292/231670842-40e1ae6c-420a-4bd3-9535-99fda5679482.png)

2020년 10월에 `61` 지역은 2위인 `79` 지역에 비해 0.3% 나 점유율이 높다. 다른 랭킹 간 차이를 비교해보면 상대적으로 엄청난 차이다.

더불어 랭킹 변동이 없는 것으로 보아 이는 `61` 지역이 압도적으로 수요가 높다는 뜻으로 볼 수 있다. 

따라서 경쟁사의 점유율을 뺏어오는 전략으로 해당 지역을 공략하는 것도 좋은 방법일 것 같다.

인기 급상승 지역 지표를 보면 해당 달에 `13` 지역은 24 순위가 증가했으며, 갑자기 수요가 늘어난 지역이라고 볼 수 있다. 

이 급작스런 수요가 어떤 원인에 의한 것인지 파악하고, 미래에도 이 수요가 증가/감소될지 분석해보는 것도 좋다.

다만 랭킹도 낮고, 점유율도 크지 않다. 

랭킹 변동성이 큰 구간이라서 유의미하진 않아 보인다.

오히려 `34`위인 `161` 지역을 추가 분석해보는게 나아 보인다.

<br/>

## 분석에 대한 시사점

사실 프로젝트에서 분석한 지역 수요 분석은 제대로 된 수요 분석은 아니다. 

운행 기록 데이터를 기반으로 분석했기 때문이다.

콜 요청이 있었지만 택시 매칭이 안되어 결국 사용자가 취소하게 되었다던지 다양한 변수가 있을 수 있기 때문이다.

따라서 유저 행동 데이터를 기반으로 수요 분석하는게 올바를 것이다.

(콜 요청부터 최종 목적지 도착까지를 한 시퀀스로 볼 때, 정상 시퀀스율이 높다면 어느 정도 유의미하겠지만)

다만, 이 프로젝트에서는 데이터를 보고 랭킹, 점유율, 전월 대비 변동 등 이러한 관점으로도 분석할 수 있다는 방법론만 제시할 뿐이다.
